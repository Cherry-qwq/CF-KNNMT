    ...
    def forward(
        ...
        if self.args.knn_mode == "build_datastore":
            ...
            for it in x:
                ...
                self.retriever.retrieve(x2, return_list=["indices","distances"] ,k = select)
                ...
                everydic2 = self.retriever.results["distances"]#(batch_size,select)
                everydic = everydic2.cpu()  # 将 tensor 移动到 CPU
                everydic = everydic.view(-1) #我猜他是一长串，分别对应
                ...
                selected_k = max(kdic, key=lambda k: kdic[k])
                with open(os.path.join('/data/qirui/z-testdata','example_selected_k.txt'), 'a') as file:
                    string = f"first:{selected_k} second:{kdic}"+"\n"
                    file.write(string) 
